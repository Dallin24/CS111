...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    >>> gridy = Grid.build([[None, 's',   'r'], [None, None, None]])
    >>> is_move_ok(gridy, 1, 0, 1, 1)
    True
    """
    if (y_to >= grid.height or x_to >= grid.width or x_to < 0):
        return False
    current_spot = grid.get(x_from, y_from)
    destination_spot = grid.get(x_to, y_to)
    if ((destination_spot != None)):
        return False
    if(x_from != x_to and (grid.get(x_to, y_to - 1) != None)):
        return False
    return True
...
def do_move(grid, x_from, y_from, x_to, y_to):
    ...
    grid.set(x_to, y_to, current_spot)
...
def do_whole_grid(grid):
    ...
    :return a modified grid
    """
    for y in reversed(range(grid.height - 1)):
