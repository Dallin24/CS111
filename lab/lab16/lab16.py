...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search('CS111|CS 111|C S 111|S111R', post))
...
def roman_numerals(text):
    ...
    []
    """
    return re.findall(r'\b[MDCLXVI]+\b', text)
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'\b(?:[01]?\d|2[0-3]):[0-5]\d[AM|PM]*\b', text)
...
def most_common_code(text):
    ...
    num_list = re.findall(r'\b\(?(\d{3})\)?-?\s?\d{3}-?\s?\d{4}\b', text)
    count_dictionary = {}
    for value in num_list:
        count_dictionary[value] = num_list.count(value)
